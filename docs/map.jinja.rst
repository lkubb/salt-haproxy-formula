...
We will see a quick setup to configure the ``haproxy`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``haproxy`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``haproxy`` formulas under ``/srv/salt/haproxy/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/haproxy/parameters/dns:domain/
    ln -s dns:domain /srv/salt/haproxy/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/haproxy/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/haproxy/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``haproxy`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``haproxy`` formulas under ``/srv/salt/haproxy/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/haproxy/parameters/roles
...
We create a configuration for the role ``haproxy/server`` in ``/srv/salt/haproxy/parameters/roles/haproxy/server.yaml``:
...
We create another configuration for the role ``haproxy/client`` in ``/srv/salt/haproxy/parameters/roles/haproxy/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles haproxy/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles haproxy/client
...
.. code-block:: console
    ...
    mv /srv/salt/haproxy/parameters/dns:domain/  '/srv/salt/haproxy/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-haproxy-as-json:
      test.nop:
        - name: {{ haproxy | json }}
...
Here is an example based on `template-formula/haproxy/config/file.sls`_:
    ...
    haproxy-config-file-file-managed:
      file.managed:
        ...
        - source: {{ files_switch(['example.tmpl'],
                                  lookup='haproxy-config-file-file-managed'
                     )
                  }}
        - mode: 644
        - user: root
        - group: {{ haproxy.rootgroup }}
        - makedirs: True
        - template: jinja
        - require:
...
This ``sls`` file expose a ``haproxy`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ haproxy['winner'] }}
...
.. _template-formula/haproxy/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/haproxy/config/file.sls
